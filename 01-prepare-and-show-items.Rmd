...
fb_ca_low <- readPNG(paste0(fact_box_dir, grep(".*ca_.*_low.*", fact_box_imgs, value = TRUE)))
...
print_cat_items(nppi_context, "ca_low", "item", "print") %>% 
  asterik_name_remover() %>% cat()
# Image
np_ca_low <- readPNG(paste0(new_paradigm_dir, grep(".*ca_.*_low.*", new_paradigm_imgs, value = TRUE)))
...
print_cat_items(fbpi_context, "high", "item", "print") %>% 
  asterik_name_remover() %>% cat()
# Image
fb_ca_high <- readPNG(paste0(fact_box_dir, grep(".*ca_.*_high.*", fact_box_imgs, value = TRUE)))
...
# Image
np_ca_high <- readPNG(paste0(new_paradigm_dir, grep(".*ca_.*_high.*", new_paradigm_imgs, value = TRUE)))
...
print_cat_items(fbpi_context, "low", "item", "print") %>% asterik_name_remover() %>% cat()
# Image
fb_pr_low <- readPNG(paste0(fact_box_dir, grep(".*pr_.*_low.*", fact_box_imgs, value = TRUE)))
...
# Image
np_pr_low <- readPNG(paste0(new_paradigm_dir, grep(".*pr_.*_low.*", new_paradigm_imgs, value = TRUE)))
...
print_cat_items(fbpi_context, "high", "item", "print") %>% asterik_name_remover() %>% cat()
# Image
fb_pr_high <- readPNG(paste0(fact_box_dir, grep(".*pr_.*_high.*", fact_box_imgs, value = TRUE)))
...
# Image
np_pr_high <- readPNG(paste0(new_paradigm_dir, grep(".*pr_.*_high.*", new_paradigm_imgs, value = TRUE)))
...
# invisible(lapply(fu_questions, function(x) {cat(remove_placeholders(x, item_followup = "followup")); cat("\n****************************\n")}))
```
...
```{r}
# TODO: html codes to insert images are not allowed on ED. if an asci tag is used it will be interpreted after ED fields calls. Can it be used with images? YASS
# TODO: remove ppv question from ED blocks and put it on trial block (with display logic). remember to put text between "" when if ED is equal to. 
