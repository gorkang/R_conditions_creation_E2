...
prev_x_ca <- 0.05852174
prev_x_pr1 <- 0.13352174
prev_x_pr2 <- 0.05852174
prev_y_1 <- 0.2562926
prev_y_2 <- 0.2792926
# absolute
...
magick::image_annotate(
  ...
  magick::image_annotate(
    ...
    paste0("At age XX, it is estimated that breast cancer is present in XX out " ),
    font = "arial", size = 20, color = "black", boxcolor = "",
    degrees = 0, location = paste0("+", prev_xca_pos, "+", prev_y1_pos)),

  text = paste0("of every ", names(nppi_graphs[1]), " women."),
  font = "arial", size = 20, color = "black", boxcolor = "",
