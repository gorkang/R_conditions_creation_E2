...
numbers2problems <- function(problems) {
  ...
  fields_nfab <- c("prev_01", "prev_02", "hit_rate_01", "hit_rate_02", "false_positive_01", "false_positive_02")
  fields_pr <- c("prev_01", "hit_rate_01", "false_positive_01")
  ...
  for (i in 1:length(problems_x)) { # 1. LOOP PROBLEMS
    ...
    if (grepl("nfab", names(problems_x[i]))) { # IF ITEM IS NATURAL FREQ.
      ...
      for (x in 1:nrow(numbers_item_x_filt)) { # 1.1. LOOP NUMBER SET TABLE
      ...
      } else if (grepl("pr", names(problems_x[i]))) { # END: IF ITEM IS NATURAL FREQ.
        ...
        for (x in 1:nrow(numbers_item_x_filt)) { # 1.1. LOOP NUMBER SET TABLE
          ...
          for (j in 1:length(field_to_replace)) { # 1.1.1. LOOP FIELDS TO REPLACE
            ...
            item2number_list[[x]] <-
              gsub(field_to_replace[j], # to be replaced
                   paste0(numbers_item_x_filt[x,][[field_to_replace[j]]]*100), #replacement
                   item2number_list[[x]]) # current problem (LOOP 1)
      ...
      } else if (grepl("pf", names(problems_x[i]))) { # END: IF ITEM IS PROBABILITY
