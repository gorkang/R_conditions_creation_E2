...
- items: contains items templates as text files, without questions.  
- pictorial_prevalences: contains pictorial items prevalences templates  
- questions_raw: contains follow-up questions  
  
### items

This folder contains follow-up items templates files. Files must be named as the following:  
`[context]_fu.txt`  
The context should be no longer than 2 letters. For example:  
`ca_fu.txt`  
`pr_fu.txt`  
...
`[followUp_fourth_piece]`  
...  
`[followUp_end]`  

The placeholders should be used to separate unities as paragraphs. As many numbered placeholders (`[followUp_first_piece]`) as wanted be used as long the first and the last placeholders are `[followUp_start]` and `[followUp_end]`.  
Additionally, html code tags can be use to give format to items exported to QUaltrics
...
This folder contains unique prevalences for pictorial items as text files. Files must be named as follow: 
...
The text within each text file must start with a name, indicating context and presentation format between asterisk:  
`**ca_fbpi**`

Following the name the actual text of the prevalence must be in the same line and can contain simple text and/or column names of number_bayes.xls to be replace with numbers.
...
It is not possible to create slider questions using Qualtrics advanced text format. To create a slider question with an unnamed slider, the best option is to creat a descriptive text question (`[[Question:Text]]`) and manually convert it to slider on the Qualtrics website. For named sliders use the vertical single choice question (`[[Question:MC:SingleAnswer:Vertical]]`) and then convert it to slider. Here the choices text will be used as slider names.
